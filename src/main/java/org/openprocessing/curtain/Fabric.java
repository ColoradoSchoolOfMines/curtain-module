...
public class Fabric extends ProcessingModule {
	/*
...
*/
	...
	public void draw() {
		...
		mouseInfluenceScalar = ( float ) (1.0 / timeStepAmt);
	...
	void createCurtain () {
		...
		for (int y = 0; y <= curtainHeight; y++) { // due to the way particles are attached, we need the y loop on the outside
			for (int x = 0; x <= curtainWidth; x++) {
				Particle particle = new Particle(this, new PVector(midWidth + x * restingDistances, y * restingDistances + yStart));
