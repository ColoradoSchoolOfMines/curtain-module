...
public class Link {
	...
	Link (Fabric parent, Particle which1, Particle which2, float restingDist, float stiff) {
		this.parent = parent;
	...
	void constraintSolve () {
		...
		if (d > parent.curtainTearSensitivity)
			p1.removeLink(this);
	...
	void draw () {
		if (drawThis)
			parent.line(p1.position.x, p1.position.y, p2.position.x, p2.position.y);
	}
}
